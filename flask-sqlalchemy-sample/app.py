...
def select_all():
    print("========== 全件取得 ==========")
    with app.app_context():
        target_tasks_list = Task.query.all()
...
def select_filter_pk(pk):
    print("========== 1件取得 ==========")
    with app.app_context():
        target_task = Task.query.filter_by(id=pk).first()
        print("更新後 =>", target_task)
...
def update(pk):
    ...
    with app.app_context():
        target_task = Task.query.filter_by(id=pk).first()
        print("更新前 =>", target_task)
        target_task.content = "課題を変更"

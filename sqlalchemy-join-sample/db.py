...
class Shop(Base):
    ...
    # 店舗名
    shop_name = Column(String(255), unique=True, nullable=False)
...
# ▼▼▼ リスト 6-6 追加 ▼▼▼
# filter
print("■：リスト6-6 filterの動作")
target_A = (
    session.query(Item)
    .filter(Item.item_id == 1)
    .filter(Item.price == 100)
    .first()
)
print(f"商品ID：{target_A.item_id} -> 価格：{target_A.price}")

# filter_by
print("■：リスト6-6 filter_byの動作")
target_B = session.query(Item).filter_by(item_id=1, price=100).first()
print(f"商品ID：{target_B.item_id} -> 価格：{target_B.price}")
# ▲▲▲ リスト 6-6 追加 ▲▲▲
